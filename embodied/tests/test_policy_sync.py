...
def test_sync_due_per_mode(mode, steps):
  ...
  if mode == 'immediate':
    ...
    assert agent._policy_sync_due(_obs(False, batch=3) | {
        'is_last': np.array([False, True, False])}, 0), (
...
def test_precompile_does_not_disturb_scheduled_sync():
  agent = _agent('steps', steps=3)
  agent.init_policy = lambda batch: {
      'state': [np.zeros((1,), np.float32)] * batch}
