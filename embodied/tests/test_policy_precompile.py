...
def _args(actor_batch):
  return types.SimpleNamespace(
      actor_batch=actor_batch, log_every=0, actor_threads=1,
      logger_addr='logger', replay_addr='replay', actor_addr='actor')
...
def test_actor_warms_policy_after_restore_before_serving(monkeypatch):
  ...
  assert events == [
      'init_policy', 'checkpoint_restored', 'warmup:3', 'warmup_complete',
      'learner_continues', 'server_construct', 'server_start',
  ], events
